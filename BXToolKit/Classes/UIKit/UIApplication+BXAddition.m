...
@interface XDSApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end

#define BX_NETWORK_INDICATOR_DELAY (1/30.0)
@implementation XDSApplicationNetworkIndicatorInfo
@end


@implementation UIApplication (BXAddition)
...
BX_SYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo, setNetworkActivityInfo, RETAIN_NONATOMIC, XDSApplicationNetworkIndicatorInfo *);
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        bx_dispatch_async_on_main_queue(^{
            XDSApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [XDSApplicationNetworkIndicatorInfo new];
            ...
            info.timer = [NSTimer timerWithTimeInterval:BX_NETWORK_INDICATOR_DELAY target:self selector:@selector(_delaySetActivity:) userInfo:@(info.count > 0) repeats:NO];
