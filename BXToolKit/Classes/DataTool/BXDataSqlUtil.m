...
@implementation BXDataSqlOrderModel
@end

@implementation BXDataSqlUtil
...
+ (NSString *)getInsertDataSql:(NSDictionary *)parameters inTableName:(NSString *)tableName{
...
+ (NSString *)getUpdateDataSql:(NSDictionary *)parameters inTableName:(NSString *)tableName conditions:(NSDictionary *)conditions{
    // 参数
...
+ (NSString *)getSelectTableSqlWithTable:(NSString *)tableName conditions:(NSDictionary *)conditions orderBy:(NSArray<BXDataSqlOrderModel*>*)orderBy limit:(NSRange)range {
    ...
    if ([tableName isEqualToString:@""]) {
        NSLog(@"getSelectTableSQLWithTable selectParameters parameters: name is \"\" ");
        return nil;
    }
    ...
    if (orderBy.count > 0) {
        ...
        for (BXDataSqlOrderModel *orderModel in orderBy) {
            [order appendFormat:@"%@ ", orderModel.order_name];
            [order appendString:orderModel.order_type == NSOrderedAscending?@"ASC ":@"ASC "];
            [order appendString:@", "];
        }
